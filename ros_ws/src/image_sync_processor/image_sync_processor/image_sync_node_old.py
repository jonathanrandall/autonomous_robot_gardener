...
class ImageSyncProcessor(Node):
    ...
    def draw_instance_segmentation_masks(self, img, masks):
        ...
        cv2.addWeighted(img, 0.75, filled, 0.45, 0.0, dst=image_ret)
        return image_ret
    ...
    def draw_instance_segmentation_masks_gl(self, img, masks,dists):
        ...
        gls = (200.0 - np.array(dists))*255.0/200.0
        image_ret = img.copy()
        filled = np.zeros_like(img)  # already grayscale or single channel

        for i, mask in enumerate(masks):
            if True or i==(len(masks) -1):
                filled[mask] = [gls[i]]*3 #COLOURS[i % len(COLOURS)]
        # Blend original and filled into a composite image
        cv2.addWeighted(image_ret, 0.75, filled, 0.75, 0.0, dst=img)
        return image_ret
...
# 
            ...
            #
            overlay_combined = self.draw_instance_segmentation_masks(img_out, shifted_masks)#,dists)
